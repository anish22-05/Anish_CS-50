...
SELECT ROUND(AVG(entropy),2) AS "Hitoshige Average Entropy" FROM views WHERE artist = "Hiroshige";
...
<-- Make use of AS to rename a column,Involve at least one condition, using WHERE,Sort by at least one column, using ORDER BY.
SELECT english_title AS "Lowest Entropy" FROM views ORDER BY entropy LIMIT 1;
