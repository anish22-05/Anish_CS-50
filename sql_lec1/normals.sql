...
SELECT "0m" FROM normals WHERE longitude = -69.5 AND latitude = 42.5;
...
SELECT "225m" FROM normals WHERE longitude = -69.5 AND latitude = 42.5;
...
SELECT "0m","100m","200m" FROM normals where longitude = -101.5 AND latitude = 89.5;
...
<--as well as their respective degrees of latitude and longitude, 
<-- within the Arabian Sea. Include latitude, longitude, and temperature columns. For simplicity, 
<--assume the Arabian Sea is encased in the following four coordinates: 
<-- 20° of latitude, 55° of longitude,20° of latitude, 75° of longitude
<-- 0° of latitude, 55° degrees of longitude ,0° of latitude, 75° degrees of longitude
...
SELECT ROUND(AVG("0m"),2) AS "Average Equator Ocean Surface Temperature" FROM normals WHERE longitude = -0.5 AND latitude = 0.5;
<-- write a SQL query to find the 10 locations with the lowest normal ocean surface temperature, sorted coldest to warmest. 
...
<--If two locations have the same normal ocean surface temperature, sort by latitude, smallest to largest. 
<--Include latitude, longitude, and surface temperature columns
SELECT latitude,longitude,"0m" FROM normals ORDER BY "0m" DESC, latitude DESC LIMIT 10;
